...
export class ProductService {
  ...
  async getFilters(
    ...
    const data = Array.from(map.values())
      ...
      .map((attribute) => ({
        ...attribute,
        values: attribute.values.sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0)),
      }))
  ...
  private async replaceProductVariants(
    ...
    const hasExplicitDefault = variants.some((variant) => variant.isDefault === true);
    const createdVariants = variants.map((variant, index) =>
      variantRepository.create({
        ...variant,
        isDefault: typeof variant.isDefault === 'boolean'
          ? variant.isDefault
  ...
  private async replaceProductOptions(
    ...
    for (const option of options) {
      ...
      if (!belongsToAttribute) {
        throw new BadRequestException('مقادیر ویژگی با ویژگی انتخاب‌شده همخوانی ندارند');
      }
    }
      ...
      qb.andWhere(
        (subQb) => {
          const subQuery = subQb
            ...
            .andWhere(
              `pov_filter.attributeValueId IN (:...${valueIdsParam})`,
            )
            .getQuery();
